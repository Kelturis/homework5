...
// Твой код:
// Используем делегирование: вешаем обработчик на контейнер .pet-shop
petShop.addEventListener('click', function (event) {
  // Проверяем, что клик был именно по кнопке с питомцем (у нее есть класс .pet)
  const target = event.target;
  if (target.classList.contains('pet')) {
    // Получаем id питомца из id кнопки
    const petId = target.id;

    // Проверяем, есть ли уже такой питомец в корзине (по заданию не сказано, что нельзя добавлять повторно, но обычно так делают)
    // Если повторные добавления разрешены, то просто проверяем длину корзины
    if (cart.length >= 3) {
      // Если в корзине уже 3 питомца, показываем сообщение
      messageBox.textContent = 'Вы не можете добавить более 3 питомцев';
      return; // Выходим из обработчика, не добавляя питомца
    }
